...
print('A file with comparison counts is expected.')
print('The columns must be tab-separated.')
print('The first line giving col-names should begin with #.')

with open(Input_Folder + Name_Counts, 'r') as Counts:
    #Obtaining the names of the columns from the first line
    Column_Names = Counts.readline()
    ...
    #Obtaining the data numbers
    Counts_Data = np.loadtxt(Counts)
    #print(Counts_Data)
    Flux = Counts_Data[:,0]
    Counts_Values = Counts_Data[:,1::2]
    Counts_Errors = Counts_Data[:,2::2]
    #print(Flux)
    #print(Counts_Values)
    #print(Counts_Errors)

    N_Datasets = len(Counts_Values[0])
    print('Number of datasets read from file: ', N_Datasets)
...
for i in range(N_Datasets):
    plt.errorbar(Flux, Counts_Values[:,i], yerr = Counts_Errors[:,i], fmt = 'o-')
